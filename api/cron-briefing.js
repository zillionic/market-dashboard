...
// 텔레그램 원문에 그 업종을 구체적으로 다룬 내용이 있으면 원인→결과가 드러나는 풍부한
// 한 줄로, 없어서 업종 특성으로 추정한 경우엔 지금까지처럼 짧게 — 근거의 양에 따라
// 길이를 다르게 하도록 프롬프트에 명시합니다(원래는 항상 15~25자로 고정해서, 원문에
// 좋은 근거가 있어도 다 못 쓰고 잘려나간다는 피드백을 받았습니다).
...
아래는 오늘 실제로 상승·하락 상위로 집계된 업종 목록입니다. 각 업종별로 왜 그런 움직임을 보였는지 개조식(명사형 종결)으로 한 줄 이유를 작성해주세요. 원문에 담긴 근거의 구체성에 따라 길이를 다르게 하세요:
- 원문에 그 업종을 구체적으로 다룬 내용(원인이 된 사건·수치·발언·다른 종목이나 업종과의 연관 등)이 있으면, 그 내용을 살려 원인→결과가 논리적으로 이어지도록 30~45자 내외까지 풍부하게 쓰세요.
- 원문에 특별한 언급이 없어 업종 특성과 시황 전반만으로 추정하는 경우엔, 지금처럼 15~25자 내외로 간결하게만 쓰세요.
...
[{"name":"업종명","reason":"이유"}, ...]`;
  ...
  const res = await fetchWithTimeout("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      model: "claude-sonnet-5",
      // 원문 근거가 풍부한 업종은 최대 45자까지 늘어날 수 있어(예전 15~25자 고정 대비),
      // 업종 10개(top5+bottom5) 전부 그런 경우까지 감안해 여유를 800→1400으로 올렸습니다.
      // 예전 max_tokens 부족으로 시황 요약이 중간에 잘리던 사고(summarize())가 재발하지
      // 않도록 미리 넉넉히 잡습니다.
      max_tokens: 1400,
      messages: [{ role: "user", content: prompt }],
    }),
...
이 중에서 실제로 시장에 영향을 줄 만큼 중요한 것만 국내·해외 각각 최대 5개씩 골라주세요. 같은 사안을 다루는 중복·유사 기사는 하나만, 단순 통계 재탕이나 시장과 무관한 사건은 제외하세요. 해외 후보는 미국 증시(S&P500·나스닥·다우)와 직접 관련되거나 미국 시장에 뚜렷한 영향을 주는 것만 고르고, 인도·중국·일본 등 다른 개별 국가 증시 소식은 미국 시장에 미치는 영향이 명시적으로 드러나지 않으면 제외하세요. 후보 인덱스(예: "KR-0")로만 답하고, 전부 사소하면 해당 배열은 비워도 됩니다.
